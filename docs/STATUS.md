# STATUS — Rounds A/B/C/D/E complete + final pass (2026-09-04)

State: rounds A, B, C, D and E of `docs/IMPROVEMENTS-REPORT.md` are
implemented, verified and pushed to `origin/master`. typecheck ✓ ·
production build ✓ · visually verified in-browser after each round. This
round replaces the 2026-09-03 A/B/C STATUS. Only report item left open is
#7 (procedural WebAudio sound) — deliberately deferred, see "Next".

Commits: `a390e70` (Round A) · `68c76f2` + `0356707` (chore) · `b4dcce7`
(Round B) · `63b0480` (assets) · `19a4bf4` (Round C) · `7325cff`
(Round D+E).
...
## Round D — real star sky + asset diet (report #2, #8)

- **Real star sky** (`setup/starfield.ts`): the procedural starfield is
  replaced by 8,913 HYG catalog stars (≤ mag 6.5, RA/Dec/mag/B-V-RGB, 339
  proper names) mapped RA/Dec → unit vectors with the equatorial→ecliptic
  rotation (ε=23.44°), magnitude→size/brightness in the Points shader, plus
  88 IAU constellation figures as `LineSegments` over a galactic-aligned
  Milky Way band. GUI toggle "Constellations" (default on). Verification
  captures: `docs/screenshots/captures/round-d-01` through `-06`.
- **KTX2 asset diet** (`scripts/build-textures-ktx2.mjs`,
  `setup/texture-manifest.ts`, `setup/textures.ts`): all 30 JPG textures
  converted to KTX2/Basis with JPG fallback via the generated
  `KTX2_AVAILABLE` manifest (28 MB textures dir → ~6.7 MB download,
  ~85% less VRAM). Transcoder at `static/basis/`
  (`basis_transcoder.js` + `.wasm`).

## Round E — observatory event scanner (report #10)

- **Alignment & eclipse detector** (`setup/events.ts`): throttled
  500 ms scan over the real Keplerian positions — Sun-line planetary
  alignments, close conjunctions and eclipse geometries — surfaced as
  glass toasts with a "View" button that flies to the event's vantage
  (same flow as a palette select). Per-event cooldown so toasts don't
  flap while bodies drift. `window.__solar` exposes `eventScanner`.

## Final pass — walkthrough, screenshots, close-out (2026-09-04)

- Walked the app as a user view-by-view (Sun/system, Earth, Saturn,
  Pluto, Kuiper belt, search palette, true scale): bloom, trails,
  constellations, Milky Way, telemetry HUD, facts cards and caption
  pill all render; rail collapse, focus flights and GUI toggles work.
- README screenshots refreshed — all 7 PNGs re-captured at 1600×900
  against the current UI (the old set still showed the pre-round-A top
  toolbar). `README.md` references unchanged
  (`docs/screenshots/*.png`).
- typecheck ✓ · production build ✓ (pre-existing >500 kB chunk-size
  warning only).
...
Only one top-10 item remains, deliberately deferred:

1. **Round #7 — procedural sound design (WebAudio, zero payload)**:
   mute-by-default-able ambient layer (deep-space drone, UI ticks,
   focus-flight whoosh, quiz chimes), AudioContext lazily created on
   first user gesture, persisted mute in `localStorage`. Skipped as
   out of scope for the visual/science rounds — the roadmap recipe
   (`audio.ts` + ~10 synth presets wired into `changeFocus` / tour
   `flyTo` / quiz `answerWith`) is intact in the report when wanted.
